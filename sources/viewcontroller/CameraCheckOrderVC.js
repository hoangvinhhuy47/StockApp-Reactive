...
import ErrorPOP from '../popup/ErrolPOP.js';
import {CameraKitCameraScreen} from 'react-native-camera-kit';
import Sound from 'react-native-sound';

export default class CameraCheckOrderVC extends Component {
  constructor(props) {
    super(props);
  ...
  onBarcodeScan(barCode) {
    ...
    let url =
      global.WebAPI +
      '/API/stock/GetOrderByBarcode?GUIID=' +
      global.GUIID +
      '&code=' +
      this.valueQR +
      '&userId=' +
    ...
    axios
      ...
      .then(response => {
        if (response.status == 200) {
          ...
          if (status == 1) {
            this.props.navigation.push('CheckOrderDetail', {
              Order: data,
              isCamera: true,
            });
          } else {
