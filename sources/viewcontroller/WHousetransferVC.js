...
import ItemWHouseTransferCell from '../cells/ItemWHouseTransferCell.js';
const db = database();
...
export default class WHousetransferVC extends Component {
  constructor(props) {
    super(props);
  ...
  getStockOutTransfer = async () => {
    ...
    let url =
      global.WebAPI +
      '/API/stock/GetStockOutTransferList?GUIID=' +
      global.GUIID;
    let body = {
  ...
  deleteStockOutTransfer = async () => {
    this.setState({
      loading: true,
    });
    let url =
      global.WebAPI + '/API/stock/DeleteStockOutTransfer?GUIID=' + global.GUIID;
    let body = {
      UserID: global.UserModel.UserID,
  ...
  _deleteCheckPressed = data => {
    this.setState(
      ...
      () => {
        this.deleteStockOutTransfer();
      },
    );
  };
  _popupOK_eventdeleteTranfer = () => {
    const tempData = this.state.listOutTransfer;
    this.setState(
      {
        deleteOutTranferVisible: false,
        listOutTransfer: [],
  ...
  renderTotal() {
    if (this.state.listOutTransfer.length > 0) {
      return (
        <View
          style={{
            width: 85,
          ...
          <Text
            ...
            {this.state.listOutTransfer.length}/{this.state.totalRow}
          </Text>
        </View>
      );
    } else {
        ...
        <View style={styles.backgroudBodybody}>
          <View>
            <SwipeListView
              ...
              data={this.state.listOutTransfer}
  ...
  _itemSelected_click = data => {
    this.props.navigation.push('AddNewWHouseTransfer', {OutTransfer: data});
  };
  _back_click = () => {
    this.props.navigation.goBack();
  };
  _AddNewOutTransfer = async () => {
    Utils.getSyncDate()
      .then(res => {
        if (res !== null) {
          let currentDate = new Date();
          ...
          if (arr.length == 3) {
            if (day === arr[0] && month === arr[1] && year === arr[2]) {
              this.props.navigation.navigate('AddNewWHouseTransfer', {
                OutTransfer: null,
              });
              return;
            }
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <SafeAreaView style={styles.container}>
          <Modal
            ...
            {/* <ModalContent> */}
            <SuccessPOP
              ErrorDescription={Language.SUCECCDELETE_OUTTRANFER}
          ...
          <View style={styles.headerView}>
            <Text style={styles.headerTitle}>Xuất chuyển kho</Text>
          ...
          <View style={styles.containerBody}>
            <View style={styles.headerbody}>
              ...
              <View
                ...
                <TouchableOpacity
                  onPress={this._searchOutTransfer}
                  style={styles.findButton2}>
                  <Text style={styles.textButton}>Tìm</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={this._AddNewOutTransfer}
